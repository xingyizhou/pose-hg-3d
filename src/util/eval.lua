...
function getPreds3D(hm)
    assert(hm:size():size() == 5, 'Input must be 5-D tensor')
    local max, idx = torch.max(hm:view(hm:size(1), hm:size(2), hm:size(3) * hm:size(4) * hm:size(5)), 3)
    local preds = torch.repeatTensor(idx, 1, 1, 3):float()
    preds[{{}, {}, 1}]:apply(function(x) return (x - 1) % (hm:size(5)) + 1 end)
    preds[{{}, {}, 2}]:apply(function(x) return math.floor((x - 1) / (hm:size(5))) % (hm:size(4)) + 1 end)
    preds[{{}, {}, 3}]:add(-1):div(hm:size(4) * hm:size(5)):floor():add(1)
    return preds
end
...
function regAccuracy(output, label, thr, idxs)
    local preds = (output:float():view(output:size(1), 16, 2) + 1) * opt.outputRes / 2
    local gt = (label:float():view(output:size(1), 16, 2) + 1) * opt.outputRes / 2
    -- print('preds')
    -- print(preds)
