...
function createModel()
    ...
    for i = 1, 4 do
        for j = 1, opt.nRegModules do reg = Residual(opt.nFeats, opt.nFeats)(reg) end 
        reg = nnlib.SpatialMaxPooling(2,2,2,2)(reg)
    end                                                                     -- 4 x 4
    
    local nFeat = 4 * 4 * opt.nFeats
    reg = nn.View(nFeat):setNumInputDims(3)(reg)
    reg = nn.Linear(nFeat, 16)(reg)
